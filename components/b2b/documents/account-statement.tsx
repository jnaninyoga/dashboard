...
export type ChainInvoice = DocumentWithRelations & { payments?: B2BPayment[] };
...
export function computeChain(
	...
	const rows: ChainRow[] = invoices.map((invoice) => {
		...
		return {
			...
			isCurrent: invoice.id === currentDocumentId,
		};
	});
	...
	return {
		...
		chainPaid,
		chainDue,
		chainCleared: chainDue <= 0.005,
		settledCount: rows.filter((r) => r.isFullyPaid).length,
		totalCount: rows.length,
		currentIndex: rows.findIndex((r) => r.isCurrent),
	};
}

export function ChainInvoiceCards({ rows }: { rows: ChainRow[] }) {
	return (
		<div className="space-y-3">
			{rows.map(({ invoice, total, paid, due, isFullyPaid, isCurrent }) => {
				const cardBaseClasses = cn(
					"relative overflow-hidden rounded-2xl border px-4 py-3 transition-colors",
					{
						...
						"border-secondary-2/70 bg-secondary/20":
							!isCurrent && !isFullyPaid,
					},
				);
					...
					<div className="flex items-center justify-between gap-3">
						<div className="flex min-w-0 items-center gap-3">
							<div
								className={cn(
									"flex size-9 shrink-0 items-center justify-center rounded-xl",
									{
										...
										"bg-secondary-2/20 group-hover:bg-primary/10 transition-colors":
											!isCurrent && !isFullyPaid,
									},
								)}
							>
								<DocumentText
									...
									className={cn("transition-colors", {
										...
										"text-secondary-3 group-hover:text-primary transition-colors":
											!isCurrent && !isFullyPaid,
									})}
								/>
							</div>
							<div className="flex min-w-0 flex-col">
								<span
									className={cn(
										"truncate text-sm leading-tight font-bold transition-colors",
										isCurrent
											? "text-primary"
				...
				const summaryRow = (
					<div className="mt-2 flex items-center justify-between gap-3">
						...
						<div className="flex items-baseline gap-1.5">
							...
							<span
								className={cn(
									"font-heading text-base font-bold tracking-tight tabular-nums",
									{
										"text-muted-foreground/60 line-through decoration-current/40 decoration-1":
											isFullyPaid,
										"text-primary": isCurrent,
				...
				return (
					<Link
						...
						className={cn(
							"group hover:border-primary/30 hover:bg-primary/5 block",
							cardBaseClasses,
						)}
					>
						{inner}
...
export function ChainSummaryFooter({
	chainTotal,
	chainPaid,
	chainDue,
	chainCleared,
}: Pick<ChainSummary, "chainTotal" | "chainPaid" | "chainDue" | "chainCleared">) {
	return (
		<div className="bg-secondary/30 border-secondary-2/50 space-y-3 rounded-2xl border px-5 py-4">
			...
			<div className="border-secondary-2/50 flex items-end justify-between border-t pt-3">
				...
				>
					...
					<span className="ml-1.5 text-xs font-semibold opacity-60">MAD</span>
				</span>
			</div>
		</div>
	);
