...
export default async function PartnerDetailPage(props: { params: Params }) {
	...
	return (
		<div className="mx-auto flex w-full max-w-6xl flex-col gap-8 p-6">
			...
			<Link href="/b2b/partners" className="group text-muted-foreground hover:text-primary flex items-center gap-2 text-sm transition-colors">
			...
			<div className="border-secondary/20 bg-card relative flex flex-col justify-between gap-6 overflow-hidden rounded-3xl border p-8 shadow-sm md:flex-row">
				<div className="text-primary absolute -top-10 right-0 p-8 opacity-15">
					...
					<div className="text-muted-foreground/90 mt-2 flex flex-col gap-4 sm:flex-row">
						{partner.address ? (
							<div className="flex max-w-sm items-start gap-2">
								<Location size={18} variant="Bulk" className="text-primary/60 shrink-0" />
								<span className="text-sm leading-relaxed font-medium">{partner.address}</span>
							</div>
						) : null}
					</div>
				</div>
				...
				<div className="relative z-10 flex shrink-0 flex-col justify-center gap-3">
					<div className="border-secondary-3/20 bg-secondary/80 flex items-center gap-4 rounded-2xl border px-6 py-4 shadow-xs backdrop-blur-sm">
						<div className="text-center">
							<div className="text-foreground text-2xl font-bold">{partner.contacts?.length || 0}</div>
							<div className="text-secondary-foreground text-xs font-bold tracking-widest uppercase">Contacts</div>
						</div>
						<Separator orientation="vertical" className="mx-2 h-8" />
						<div className="text-center">
							<div className="text-foreground text-2xl font-bold">{partner.documents?.length || 0}</div>
							<div className="text-secondary-foreground text-xs font-bold tracking-widest uppercase">Docs</div>
						</div>
					</div>
				</div>
			</div>
				...
				<div className="space-y-6 lg:col-span-1">
					<div className="flex items-center justify-between px-2">
						...
						<ContactDialog partnerId={partner.id}>
							<Button size="sm" variant="outline" className="hover:bg-primary hover:text-primary-foreground rounded-full shadow-xs transition-all">
