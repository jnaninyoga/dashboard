import { pgTable, uuid, text, date, timestamp, integer, decimal, pgEnum, boolean, jsonb } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';

export const categoryEnum = pgEnum('category', ['adult', 'child', 'student']);
export const sexEnum = pgEnum('sex', ['male', 'female']);
export const referralSourceEnum = pgEnum('referral_source', ['social_media', 'website', 'friend', 'professional_network', 'other']);
export const walletStatusEnum = pgEnum('wallet_status', ['active', 'empty', 'cancelled', 'expired']);
...
export const attendanceLedger = pgTable('attendance_ledger', {
  id: uuid('id').defaultRandom().primaryKey(),
  walletId: uuid('wallet_id').references(() => clientWallets.id, { onDelete: 'cascade' }).notNull(),
